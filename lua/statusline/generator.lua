...
local providers = require('statusline.providers')
...
local function parse_sep(sep, parent_bg)
    ...
    if type(sep) == "string" then
        ...
        str = evaluate_if_function(sep.str, '')
        hl = evaluate_if_function(sep.hl, {fg = parent_bg, bg = colors.bg})
    end
...
local function parse_sep_list(sep_list, parent_bg)
    ...
    if sep_list[1] and (type(sep_list[1]) == "table" or type(sep_list[1]) == "string") then
