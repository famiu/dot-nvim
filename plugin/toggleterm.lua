...
local function toggle_term_buffer()
    if not term_buffer or not vim.api.nvim_buf_is_valid(term_buffer) then
        ...
        vim.api.nvim_buf_call(term_buffer, function()
            vim.fn.termopen(
                vim.o.shell,
                { on_exit = function() vim.api.nvim_buf_delete(term_buffer, { force = true }) end }
            )
        end)
    else
        local term_wins = vim.tbl_filter(
            function(win) return vim.api.nvim_win_get_buf(win) == term_buffer end,
