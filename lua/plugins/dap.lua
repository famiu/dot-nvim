...
keymap.set('n', '<F6>', function() dap.step_back() end, { silent = true })
...
keymap.set('n', '<Leader>b', function() dap.toggle_breakpoint() end, { silent = true })
keymap.set('n', '<Leader>B', function() dap.set_breakpoint(fn.input('Breakpoint condition: ')) end,
    { silent = true })
...
keymap.set('n', '<Leader>dl', function() dap.run_last() end, { silent = true })
...
dap.adapters.lldb = {
    type = 'executable',
    command = vim.trim(fn.system({ '/usr/bin/which', 'lldb-vscode ' })), -- adjust as needed, must be absolute path
    name = 'lldb'
}
